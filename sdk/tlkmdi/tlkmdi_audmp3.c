...
int tlkmdi_mp3_init(void)
{
	...
	tlkapi_save2_init(&sTlkMdiMp3Ctrl.save, TLKMDI_MP3_FILE_INFO_SIGN, TLKMDI_MP3_FILE_INFO_VERS,
		TLKMDI_MP3_FILE_INFO_SIZE, TLKMDI_MP3_FILE_INFO_ADDR);
...
bool tlkmdi_mp3_enable(bool enable)
{
	...
	if(enable){
		if(spTlkMdiMp3CacheBuff == NULL) spTlkMdiMp3CacheBuff = (uint08*)tlkapi_malloc(TLKMDI_MP3_DEC_CACHE_SIZE);
		if(spTlkMdiMp3ParamBuff == NULL) spTlkMdiMp3ParamBuff = (uint08*)tlkapi_malloc(TLKMDI_MP3_DEC_PARAM_SIZE);
		if(spTlkMdiMp3RawPcmBuff == NULL) spTlkMdiMp3RawPcmBuff = (uint08*)tlkapi_malloc(TLKMDI_MP3_DEC_PCM_SIZE);
...
//	tlkapi_trace(TLKMDI_AUDMP3_DBG_FLAG, TLKMDI_AUDMP3_DBG_SIGN, "tlkmdi_play_start 03");
...
int tlkmdi_mp3_loadFile(bool isForce)
{
	...
	if(tlkmdi_mp3_isEnable()) return false;
	
	sTlkMdiMp3Ctrl.playIndex = 0;
	sTlkMdiMp3Ctrl.fileCount = 0;
	tlkapi_flash_read(TLKMDI_MP3_FILE_LIST_ADDR, (uint08*)&listInfo, sizeof(tlkmdi_mp3_flist_t));
	...
	if(listInfo.sign != 0xFF || listInfo.size != 0xFF || listInfo.vers != 0xFF){
		...
		for(address=0; address<TLKMDI_MP3_FILE_LIST_LENS; address+=4096){
			flash_erase_sector(TLKMDI_MP3_FILE_LIST_ADDR+address);
		}
	}
...
//				tlkapi_array(TLKMDI_AUDMP3_DBG_FLAG, TLKMDI_AUDMP3_DBG_SIGN, "tlkmdi_mp3_scanPlayFile: this is not mp3 file", fno.fname, 13);
				...
				tlkapi_flash_write(TLKMDI_MP3_FILE_LIST_ADDR+address, (uint08*)finfo, TLKMDI_MP3_FILE_LIST_SIZE);
...
//				tlkapi_array(TLKMDI_AUDMP3_DBG_FLAG, TLKMDI_AUDMP3_DBG_SIGN, "tlkmdi_mp3_scanPlayFile: this is mp3 file", fno.fname, 13);
	...
	tlkapi_flash_write(TLKMDI_MP3_FILE_LIST_ADDR, (uint08*)&listInfo, sizeof(tlkmdi_mp3_flist_t));
...
int tlkmdi_mp3_findFileName(uint08 *pBuff, uint16 index)
{
	...
	addr = TLKMDI_MP3_FILE_LIST_ADDR+sizeof(tlkmdi_mp3_flist_t)+index*TLKMDI_MP3_FILE_LIST_SIZE;
