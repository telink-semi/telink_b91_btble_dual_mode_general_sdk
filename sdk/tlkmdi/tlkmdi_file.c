...
static int tlkmdi_file_sendRspPkt(uint08 optChn, uint16 handle, uint08 status, uint08 reason,
	uint08 cmdID, uint08 dataPort, uint08 crypSign, uint08 *pData, uint08 dataLen, uint08 *pSerial);
static int tlkmdi_file_sendEvtPkt(uint08 optChn, uint16 handle, uint08 evtID, uint08 dataPort,
	uint08 crypSign, uint08 *pData, uint08 dataLen, uint08 *pSerial);
...
static void tlkmdi_file_sendSyncShakeEvtProc(tlkmdi_file_unit_t *pUnit);
...
static void tlkmdi_file_recvDatDeal(uint08 optChn, uint16 handle, uint08 *pHead, uint16 headLen, uint08 *pData, uint16 dataLen)
{
	...
	tlkapi_trace(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_recvDatDeal: dataNumb[%d],expNumb[%d],packLens[%d],waitMask[%x]", 
		dataNumb, packNumb, packLens, pUnit->waitMask);
	
	if(packNumb != dataNumb && pUnit->packNumb != dataNumb && (pUnit->busys & TLKMDI_FILE_BUSY_SEND_SYNC_STALL_EVT) == 0){
		uint16 diffNumb;
		if(packNumb < dataNumb) diffNumb = dataNumb-packNumb;
		else diffNumb = packNumb-dataNumb;
		if(diffNumb > 64){ //Rapid correction
			pUnit->busys |= TLKMDI_FILE_BUSY_SEND_SYNC_STALL_EVT;
			tlkapi_warn(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_recvDatDeal: stall packet");
		}else if(packNumb < dataNumb || diffNumb > 32){
			pUnit->busys |= TLKMDI_FILE_BUSY_SEND_SYNC_STALL_EVT;
			tlkapi_warn(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_recvDatDeal: stall packet");
		}
		return;
...
//	tlkapi_fatal(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_recvDatDeal: dealSize[%d],fileSize[%d],packNumb[%d]",
//		pUnit->dealSize, pUnit->fileSize, packNumb);
	if(pUnit->dealSize+packLens > pUnit->fileSize || pUnit->unitLens < packLens
		...
		tlkapi_fatal(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_recvDatDeal: transfer fault");
		pUnit->state = TLKMDI_FILE_STATE_CLOSE;
...
static void tlkmdi_file_recvStartTranCmdDeal(uint08 optChn, uint16 handle, uint08 csign, uint08 *pData, uint16 dataLen)
{
	...
	if(reason != 0 || pUnit == nullptr){
		...
		buffLen += 16;
		tlkmdi_file_sendRspPkt(optChn, handle, status, reason, TLKPRT_COMM_CMDID_FILE_START_TRAN, 0x00, 0x00, buffer, buffLen, nullptr);
		tlkapi_trace(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_recvStartTranCmdDeal: failure - status[%d],reason[%x]", status, reason);
		return;
	}
...
static void tlkmdi_file_recvCryptShakeCmdDeal(tlkmdi_file_unit_t *pUnit, uint08 csign, uint08 *pData, uint16 dataLen)
{
	...
	pUnit->flags = TLKMDI_FILE_FLAG_WAIT_SET_DATA_SCH | TLKMDI_FILE_FLAG_WAIT_SET_FILE_NAME | TLKMDI_FILE_FLAG_WAIT_START_DATA;
	if(pUnit->cryptSch == 0){
		pUnit->csign = TLKPRT_FILE_CSIGN_NONE;
	}else{
...
static bool tlkmdi_file_timer(tlkapi_timer_t *pTimer, uint32 userArg)
{
	...
	if(pUnit->idleTime == 0){
		...
		if(sTlkMdiFileCtrl.recvInfs.close != nullptr && (pUnit->flags & TLKMDI_FILE_FLAG_START) != 0){
			...
			}else if(pUnit->status == 0){
				sTlkMdiFileCtrl.recvInfs.close(pUnit, TLKMDI_FILE_TRAN_STATUS_PAUSED);
			}else{
...
static void tlkmdi_file_clientSendProc(tlkmdi_file_unit_t *pUnit)
{
	if((pUnit->busys & TLKMDI_FILE_BUSY_SEND_START_TRAN) != 0){
		...
		pUnit->busys &= ~TLKMDI_FILE_BUSY_SEND_SYNC_SHAKE_EVT;
		tlkmdi_file_sendSyncShakeEvtProc(pUnit);
...
static void tlkmdi_file_sendStartTranRspProc(tlkmdi_file_unit_t *pUnit)
{
	...
	if(tlkmdi_file_sendRspPkt(pUnit->optChn, pUnit->handle, status, reason, TLKPRT_COMM_CMDID_FILE_START_TRAN, 
		0x00, TLKPRT_FILE_CSIGN_NONE, buffer, dataLen, &pUnit->serial) == TLK_ENONE){
...
static void tlkmdi_file_sendStartAuthRspProc(tlkmdi_file_unit_t *pUnit)
{
	...
	dataLen += 16;
	if(tlkmdi_file_sendRspPkt(pUnit->optChn, pUnit->handle, status, reason, TLKPRT_COMM_CMDID_FILE_START_AUTH, 
		pUnit->dataPort, TLKPRT_FILE_CSIGN_AUTH, buffer, dataLen, &pUnit->serial) == TLK_ENONE){
		tlkapi_trace(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_sendStartAuthRspProc");
...
static void tlkmdi_file_sendSyncShakeEvtProc(tlkmdi_file_unit_t *pUnit)
{
	...
	dataLen = 0;
	buffer[dataLen++] = (pUnit->packNumb & 0xFF);
	buffer[dataLen++] = (pUnit->packNumb & 0xFF00) >> 8;
	buffer[dataLen++] = (pUnit->packNumb & 0xFF0000) >> 16;
	buffer[dataLen++] = (pUnit->packNumb & 0xFF000000) >> 24;
	if(tlkmdi_file_sendEvtPkt(pUnit->optChn, pUnit->handle, TLKPRT_COMM_EVTID_FILE_SHAKE, pUnit->dataPort, 
		csign, buffer, dataLen, &pUnit->serial) == TLK_ENONE){
...
static void tlkmdi_file_sendSyncStallEvtProc(tlkmdi_file_unit_t *pUnit)
{
	...
	if(tlkmdi_file_sendEvtPkt(pUnit->optChn, pUnit->handle, TLKPRT_COMM_EVTID_FILE_SYNC, pUnit->dataPort,
		csign, buffer, dataLen, &pUnit->serial) == TLK_ENONE){
		tlkapi_trace(TLKMDI_FILE_DBG_FLAG, TLKMDI_FILE_DBG_SIGN, "tlkmdi_file_sendSyncShakeEvtProc");
	}else{
		pUnit->busys |= TLKMDI_FILE_BUSY_SEND_SYNC_STALL_EVT;
...
static void tlkmdi_file_resetRecvUnit(tlkmdi_file_unit_t *pUnit)
{
	tlkmdi_adapt_removeTimer(&pUnit->timer);
