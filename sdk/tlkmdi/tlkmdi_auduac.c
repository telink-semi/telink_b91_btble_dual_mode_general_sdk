...
static void tlkmdi_auduac_spkStatusChangeEvt(bool enable, uint08 channel, uint08 bitDepth, uint32 sampleRate)
{
	...
	if(enable){
		...
		if(!sTlkMdiAudUacCtrl.enable) tlkmdi_audio_sendStartEvt(TLKPRT_COMM_AUDIO_CHN_UAC, 0xFFFF);
	}else{
		if(!sTlkMdiAudUacCtrl.micEn){
			tlkmdi_auduac_switch(0xFFFF, TLK_STATE_CLOSED);
			tlkmdi_audio_sendCloseEvt(TLKPRT_COMM_AUDIO_CHN_UAC, 0xFFFF);
		}
	}
}
static void tlkmdi_auduac_micStatusChangeEvt(bool enable, uint08 channel, uint08 bitDepth, uint32 sampleRate)
...
static void tlkmdi_uac_spkHandler(void)
{
	...
	while(true){
		length = 0;
		if(tlkdev_codec_getSpkIdleLen() > 64){
			length = tlkusb_uacspk_readData(buffer, 64, true);
		}
  		if(length == 0) break;
