 ...
 * Descript: start a Inquiry.
 * Params:
 *        @inqType[IN]--The inquiry type.
 *        @rssiThd[IN]--The rssi of signal.
 *        @maxNumb[IN]--The report devices number.
 *        @timeout[IN]--The total inquiry time.
 *        @isGetName[IN]--whether get Bt device name or not.
 * Return: Return TLK_ENONE is success,other value is false.
 * Others: None.
*******************************************************************************/
int  tlkmdi_btinq_start(uint08 inqType, uint08 rssiThd, uint08 maxNumb, uint08 inqWind, uint08 inqNumb, bool isGetName)
{
	uint08 stage;
	
	if(inqWind > 100 || inqWind < 3) return -TLK_EPARAM;
...
void tlkmdi_btinq_close(void)
{
	...
	if(stage == TLKMDI_BTINQ_CLOSING_STAGE_INQUIRY_OVER){
		...
		tlkmdi_adapt_removeTimer(&sTlkMdiBtInqCtrl.timer);
	}
...
//	tlkapi_trace(TLKMDI_BTINQ_DBG_FLAG, TLKMDI_BTINQ_DBG_SIGN, "tlkmdi_btinq_timer: timeout-%d", sTlkMdiBtInqCtrl.runTimeout);
	do{
		tlkmdi_btinq_item_t *pItem = tlkmdi_btinq_getReportItem();
		if(pItem != nullptr){
...
static void tlkmdi_btinq_inquiryProcs(void)
{
	if(sTlkMdiBtInqCtrl.stage == TLKMDI_BTINQ_INQUIRY_STAGE_CANCEL){
		...
		if(sTlkMdiBtInqCtrl.inqNumb == 0 || sTlkMdiBtInqCtrl.curNumb >= sTlkMdiBtInqCtrl.maxNumb){
			...
			sTlkMdiBtInqCtrl.stage = TLKMDI_BTINQ_STAGE_NONE;
			sTlkMdiBtInqCtrl.timeout = 0;
		}else if(bth_hci_sendInquiryCmd(INQUIRY_LENGTH_12S8, 10) == TLK_ENONE){ //set page get name 4s timeout
...
static void tlkmdi_btinq_reportDevice(tlkmdi_btinq_item_t *pItem)
{
	...
	buffLen = 0;
	buffer[buffLen++] = pItem->dtype;
	buffer[buffLen++] = pItem->rssi;
	tmemcpy(buffer+buffLen, pItem->btaddr, 6);
	buffLen += 6;
	if(pItem->nameLen > 64) pItem->nameLen = 64;
	buffer[buffLen++] = pItem->nameLen;
	if(pItem->nameLen != 0 && pItem->btname != nullptr){
		tmemcpy(buffer+buffLen, pItem->btname, pItem->nameLen);
		buffLen += pItem->nameLen;
	}
	if(tlkmdi_comm_sendBtEvt(TLKPRT_COMM_EVTID_BT_INQUIRY, buffer, buffLen) == TLK_ENONE){
		pItem->state = TLKMDI_BTINQ_ITEM_STATE_OVER;
	}else{
		pItem->state = TLKMDI_BTINQ_ITEM_STATE_WAIT;
	}
