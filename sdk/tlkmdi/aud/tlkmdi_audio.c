...
{
	tlkmdi_audio_setToneRawVolume(volume);
}
...
{
	if(sTlkMdiAudioMusicVolumeIndex >= 32){
		sTlkMdiAudioMusicVolumeIndex = 32;
		return;
	} 
	sTlkMdiAudioMusicVolumeIndex += 2;
	if(sTlkMdiAudioMusicVolumeIndex >= 34) sTlkMdiAudioMusicVolumeIndex = 32;
	tlkmdi_audio_sendVolumeChangeEvt(TLKPRT_COMM_VOLUME_TYPE_MUSIC, tlkmdi_audio_getMusicRawVolume(false));
}
...
{
	tlkmdi_audio_setMusicRawVolume(volume);
}
...
{
	tlkmdi_audio_setVoiceRawVolume(volume);
}
...
{
	tlkmdi_audio_setHeadsetRawVolume(volume);
}
