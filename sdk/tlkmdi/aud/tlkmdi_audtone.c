...
bool tlkmdi_audtone_switch(uint16 handle, uint08 status)
{
	...
	if(enable){
			...
			sTlkMdiToneCtrl.pReadBuff = tlkapi_malloc(sTlkMdiToneCtrl.encodeLen);
			sTlkMdiToneCtrl.pSaveBuff = tlkapi_malloc(sTlkMdiToneCtrl.frameLen);
			if(sTlkMdiToneCtrl.pReadBuff == nullptr || sTlkMdiToneCtrl.pSaveBuff == nullptr){
				if(sTlkMdiToneCtrl.pReadBuff != nullptr){
					tlkapi_free(sTlkMdiToneCtrl.pReadBuff);
					sTlkMdiToneCtrl.pReadBuff = nullptr;
				}
				if(sTlkMdiToneCtrl.pSaveBuff != nullptr){
					tlkapi_free(sTlkMdiToneCtrl.pSaveBuff);
					sTlkMdiToneCtrl.pSaveBuff = nullptr;
				}
				return false;
			}
			#if (TLKMDI_AUDTONE_DECODE_SBC)
			sTlkMdiToneCtrl.pDecBuff = tlkapi_malloc(TLKMDI_AUDTONE_SBC_DEC_BUFF_SIZE);
		...
		tlkdev_codec_close();
			...
			#if (TLKMDI_AUDTONE_DECODE_SBC)
			if(sTlkMdiToneCtrl.pDecBuff != nullptr){
				tlkapi_free(sTlkMdiToneCtrl.pDecBuff);
