...
#define ATT_CMD_POSTIVE_ACTION_LABEL_LENGTH				48///固定长度，主动消息
...
#define MAX_DATA_SRC_NUM			((3 * ATT_ID_NUM) + ATT_CMD_APP_ID_LENGTH + ATT_CMD_TITLE_LENGTH \
									...
									+ ATT_CMD_DATE_LENGTH + ATT_CMD_POSTIVE_ACTION_LABEL_LENGTH \
...
}ancs_sevice_state_t;///ANCS在连接初始用于建立ANCS服务时会用到的各种状态
...
typedef struct{
	u8 	attId;
	u16 len;
	u8 	data[ATT_CMD_POSTIVE_ACTION_LABEL_LENGTH];
}ancs_attId_Postive_Action_Label;
...
typedef struct{
	...
	ancs_attId_Postive_Action_Label		positiveActionLabel;
