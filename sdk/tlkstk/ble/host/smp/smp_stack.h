...
#define		MASTER_BOND_DEVICE_SEARCH_IN_SRAM_EN	1  //temporarily workaround for SiYou's BT problem(need figure out reason)

#if (MASTER_BOND_DEVICE_SEARCH_IN_SRAM_EN)
	typedef struct {
		u8 adr_type;
		u8 address[6];
		u8 rsvd;
	} mac_adr_t;
	#define SMP_MASTER_BONDING_DEVICE_MAX_NUM		2
#else
...
#if (MASTER_BOND_DEVICE_SEARCH_IN_SRAM_EN)
	mac_adr_t  bond_device[SMP_MASTER_BONDING_DEVICE_MAX_NUM];
#endif
...
//0x18
	u8 pairing_status[BLMS_MAX_CONN_NUM];
