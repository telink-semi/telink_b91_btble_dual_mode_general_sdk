...
/**
 * @brief      This function is used to obtain the currently unused cache TX FIFO numbers.
 * @param      none
 * @return     none
 */
u8			blt_llms_get_cache_tx_fifo_unused_num (void);
...
/**
 * @brief       this function is responsible for BLE task timing scheduling
 * @param[in]	sequence_no -
 * @param[in]	link -
 * @return      the time point when the scheduler's current task ends in the future( tick)
 */
u32 		blc_ll_system_tick_acl_conn_isr(u8 sequence_no, int link);


/**
 * @brief       this function is used to initialize cache TX FIFO, to workaround problem caused by
 *         	     "(ACL_xxx_TX_FIFO_SIZE * (ACL_xxx_TX_FIFO_NUM - 1)) must be less than 4096 (4K)"
 * @param[in]   pTxbuf - cache TX FIFO buffer address.
 * @param[in]   fifo_size - must use 260 now
 * @param[in]	fifo_number - 16 or 32
 * @return      none
 */
