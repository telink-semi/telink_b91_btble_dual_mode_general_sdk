...
typedef enum{
	...
	BTP_L2CAP_OPTION_FLAG_FCS     = 0x02, //It is non-negotiable. The FCS option shall only be used when the mode is being, or is already configured to Enhanced Retransmission mode or Streaming mode.
...
typedef struct{
	...
	bth_l2cap_acldata_t aclData[TLK_STK_BTACl_NUMB];
}bth_l2cap_ctrl_t;
...
typedef union{
	...
	struct{
		uint16 Type:1;   //0:I-Frame, 1-S-Frame
		uint16 RFU0:1;   //
		uint32 S:2;      //Supervisory function. 00-RR,Receiver Ready; 01-REJ,Reject; 10-RNR,Receiver Not Ready; 11-SREJ,Select Reject
		uint16 RFU1:3;   //
		uint16 R:1;      //Retransmission Disable Bit. 0-Normal operation; 1-Receiver side requests sender to postpone retransmission of I-frames.
		uint16 ReqSeq:6; //Receive Sequence Number
		uint16 RFU2:2;   //
	}field;
...
typedef union{
	...
	struct{
		...
		uint16 RFU0:1;   //
		uint32 S:2;      //Supervisory function. 00-RR,Receiver Ready; 01-REJ,Reject; 10-RNR,Receiver Not Ready; 11-SREJ,Select Reject
		uint16 P:1;      //Retransmission Disable Bit. 0-Normal operation; 1-Receiver side requests sender to postpone retransmission of I-frames.
		uint16 RFU1:2;   //Poll. The P-bit is set to 1 to solicit a response from the receiver. The receiver shallrespond immediately with a frame with the F-bit set to 1.
...
int bth_l2cap_sendIFrameSFrame(uint16 connHandle, uint16 scid, 
	uint32 ctrl, uint08 *pHead, uint16 headLen, uint08 *pData, uint16 dataLen,
	bool isAddFcs, bool isExtCtr);
int bth_l2cap_sendIFrame(uint16 connHandle, uint16 scid, uint16 ictrl, 
	uint8_t *pHead, uint16 headLen, uint8_t *pData, uint16 dataLen, bool isAddFcs);
int bth_l2cap_sendExtIFrame(uint16 connHandle, uint16 scid, uint16 ictrl, 
	uint8_t *pHead, uint16 headLen, uint8_t *pData, uint16 dataLen, bool isAddFcs);
int bth_l2cap_sendIFrameWithoutCtrFcs(uint16 connHandle, uint16 scid,
	uint8_t *pHead, uint16 headLen, uint8_t *pData, uint16 dataLen);

int bth_l2cap_sendSFrame(uint16 connHandle, uint16 scid, uint16 sctrl,
	uint8_t *pHead, uint16 headLen, uint8_t *pData, uint16 dataLen, bool isAddFcs);
int bth_l2cap_sendExtSFrame(uint16 connHandle, uint16 scid, uint16 sctrl,
	uint8_t *pHead, uint16 headLen, uint8_t *pData, uint16 dataLen, bool isAddFcs);
