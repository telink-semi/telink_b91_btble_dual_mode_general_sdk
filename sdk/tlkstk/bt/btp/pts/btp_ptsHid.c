...
static uint btp_ptshid_getReportCB(uint16 aclHandle, uint08 reportType, uint08 reportID, uint08 *pBuff, uint16 *pBuffLen);
...
//	btp_hid_setRtnMode(BTH_L2CAP_SIG_RTNMODE_ENHANCE);
    btp_hidd_regCB(btp_ptshid_setReportCB, btp_ptshid_getReportCB,
    	btp_ptshid_setProtocolCB, btp_ptshid_getProtocolCB,
		nullptr, btp_ptshid_irqRecvDataCB);
...
static uint btp_ptshid_getReportCB(uint16 aclHandle, uint08 reportType, uint08 reportID, uint08 *pBuff, uint16 *pBuffLen)
{
	...
	buffLen = *pBuffLen;
	tlkapi_trace(BTP_PTSHID_DBG_FLAG, BTP_PTSHID_DBG_SIGN, 
