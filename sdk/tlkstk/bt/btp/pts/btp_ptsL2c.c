...
static btp_ptsl2c_ctrl_t sBtPtsL2capCtrl;
...
int btp_ptsl2c_init(void)
{
	...
	tmemset(&sBtPtsL2capCtrl, 0, sizeof(btp_ptsl2c_ctrl_t));
	for(index=0; index<BTP_PTSL2C_BUFFER_LENGTH; index++){
		sBtPtsL2capCtrl.buffer[index] = index+1;
	}
	
	return bth_l2cap_regServiceCB(BTP_PSMID_PTS_L2C, btp_ptsl2c_eventCB, btp_ptsl2c_rdataCB);
}

void btp_ptsl2c_enableRtn(bool isEnable, uint08 rtnMode)
{
	sBtPtsL2capCtrl.user.enRtn = isEnable;
...
{
	sBtPtsL2capCtrl.user.enFcs = isEnable;
	sBtPtsL2capCtrl.user.fcsType = fcsType;
}
void btp_ptsl2c_enableEfs(bool isEnable)
{
	sBtPtsL2capCtrl.user.enEfs = isEnable;
}
...
int btp_ptsl2c_connect(uint16 handle)
{
	...
	if(ret != TLK_ENONE && ret != -TLK_EBUSY){
		...
		if(pChannel != nullptr){
			if(sBtPtsL2capCtrl.user.enRtn){
				bth_signal_enableRtnOption(handle, pChannel->scid, true);
				bth_signal_setRtnMode(handle, pChannel->scid, sBtPtsL2capCtrl.user.rtnMode);
			}
			if(sBtPtsL2capCtrl.user.enFcs){
				bth_signal_enableFcsOption(handle, pChannel->scid, true);
				bth_signal_setFcsType(handle, pChannel->scid, sBtPtsL2capCtrl.user.fcsType);
			}
			if(sBtPtsL2capCtrl.user.enEfs){
...
void btp_ptsl2c_destroy(uint16 handle)
{
	...
	tlkapi_trace(BTP_PTSL2C_DBG_FLAG, BTP_PTSL2C_DBG_SIGN, "btp_ptsl2c_destroy: handle[%d] state[%d] chnID[0x%x]",
		handle, pPtsL2c->state, pPtsL2c->chnID);
...
int btp_ptsl2c_sendTestData(uint16 dataLen)
{
	...
	if(dataLen == 0){
		tlkapi_error(BTP_PTSL2C_DBG_FLAG, BTP_PTSL2C_DBG_SIGN, "btp_ptsl2c_sendTestData: paramError - dataLen[%d]", dataLen);
		return -TLK_EFAIL;
	}
	if(dataLen > BTP_PTSL2C_BUFFER_LENGTH){
		tlkapi_warn(BTP_PTSL2C_DBG_FLAG, BTP_PTSL2C_DBG_SIGN, "btp_ptsl2c_sendTestData: send length is too long [%d-%d]",
	...
	if(pPtsL2c->rtnMode == BTH_L2CAP_SIG_RTNMODE_BASIC){
		return bth_l2cap_sendChannelData(pPtsL2c->handle, pPtsL2c->chnID, nullptr, 0, sBtPtsL2capCtrl.buffer, dataLen);
	}else{
		int ret;
		bth_l2cap_iframeEnhCtrlField_t *pICtrl;
		ret = bth_l2cap_sendIFrame(pPtsL2c->handle, pPtsL2c->chnID, pPtsL2c->ehnCtrl, 
			nullptr, 0, sBtPtsL2capCtrl.buffer, dataLen, pPtsL2c->fscIsEn);
		if(ret == TLK_ENONE){
...
static bool btp_ptsl2c_timer(tlkapi_timer_t *pTimer, uint32 userArg)
{
	...
	if(pPtsL2c->timeout != 0) pPtsL2c->timeout --;
	...
	if(pPtsL2c->busys != 0) btp_ptsl2c_procs(pPtsL2c);
	
	if(pPtsL2c->busys != 0 || pPtsL2c->timeout != 0) return true; //repeat
	return false; //stop
}
static void btp_ptsl2c_procs(btp_ptsl2c_item_t *pPtsL2c)
{
	if((pPtsL2c->busys & BTP_PTSL2C_BUSY_WAIT_ACK) != 0){
		if(pPtsL2c->rtnNumb != 0) pPtsL2c->rtnNumb--;
		if(pPtsL2c->rtnNumb == 0 && pPtsL2c->rtnTxNum != 0){
			bth_l2cap_sframeEnhCtrlField_t sfield = {0};
			bth_l2cap_iframeEnhCtrlField_t *pICtrl1;
			pPtsL2c->rtnNumb = pPtsL2c->rtnTime;
			if(pPtsL2c->rtnTxNum != 0) pPtsL2c->rtnTxNum--;
			...
			bth_l2cap_sendSFrame(pPtsL2c->handle, pPtsL2c->chnID, sfield.value, nullptr, 0, nullptr, 0, pPtsL2c->fscIsEn);
		}
		if(pPtsL2c->rtnTxNum == 0){
			if(pPtsL2c->monNumb != 0) pPtsL2c->monNumb --;
...
static int btp_ptsl2c_eventCB(uint08 evtID, uint16 psmID, uint08 *pData, uint16 dataLen)
{
	...
	if(evtID == BTH_L2CAP_EVTID_REQUEST){
		...
		if(pPtsL2c->handle != 0) return -TLK_EQUOTA;
		if(sBtPtsL2capCtrl.user.enRtn){
			bth_signal_enableRtnOption(pEvt->handle, pEvt->scid, true);
			bth_signal_setRtnMode(pEvt->handle, pEvt->scid, sBtPtsL2capCtrl.user.rtnMode);
		}
		if(sBtPtsL2capCtrl.user.enFcs){
			bth_signal_enableFcsOption(pEvt->handle, pEvt->scid, true);
			bth_signal_setFcsType(pEvt->handle, pEvt->scid, sBtPtsL2capCtrl.user.fcsType);
		}
		if(sBtPtsL2capCtrl.user.enEfs){
		...
		if(pPtsL2c->rtnMode != BTH_L2CAP_SIG_RTNMODE_BASIC){
			...
			if(pPtsL2c->monTime == 0) pPtsL2c->monTime = 5;
			pPtsL2c->rtnTime = rtnTime/BTP_TIMER_TIMEOUT_MS;
			if(pPtsL2c->rtnTime == 0) pPtsL2c->rtnTime = 3;
			bth_signal_getRtnCount(pEvt->handle, pEvt->scid, &pPtsL2c->rtnMaxTx);
			if(pPtsL2c->rtnMaxTx == 0) pPtsL2c->rtnMaxTx = 1;
		}
		/******************************************************************************
		...
		*******************************************************************************/
		if(bth_signal_getFcsType(pEvt->handle, pEvt->scid, &fcsType) != TLK_ENONE){
			...
			if(fcsType == BTH_L2CAP_SIG_FCSTYPE_16bFCS) pPtsL2c->fscIsEn = true;
			else pPtsL2c->fscIsEn = false;
		}
...
static void btp_ptsl2c_rdataCB(uint16 psmID, uint16 chnID, uint16 handle, uint08 *pData, uint16 dataLen)
{
	...
	if(pPtsL2c->rtnMode == BTH_L2CAP_SIG_RTNMODE_ENHANCE){
		...
		ehnCtrl = (((uint16_t)pData[1])<<8) | pData[0];
		...
		if(pICtrl1->field.ReqSeq != pICtrl0->field.TxSeq
			|| pICtrl1->field.TxSeq != pICtrl0->field.ReqSeq){
			tlkapi_error(BTP_PTSL2C_DBG_FLAG, BTP_PTSL2C_DBG_SIGN, 
				"btp_ptsl2c_rdataCB: ExpSeq[%d],RcvSeq[%d]",
				pICtrl1->field.ReqSeq, pICtrl0->field.ReqSeq);
			{
				bth_l2cap_sframeEnhCtrlField_t sfield = {0};
				sfield.field.P = 0;
