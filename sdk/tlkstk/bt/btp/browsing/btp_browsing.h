...
typedef enum{
	...
	BTP_BROWSING_FEATURE_BIT_NOW_PLAYING = 65, //This bit is set if the player supports the NowPlaying folder. Note that for all players that support browsing this bit shall be set.
...
typedef enum{
	...
	BTP_BROWSING_SCOPE_MEDIA_PLAYER_VIRTUAL_FILESYSTEM = 0x01, //Folder Item(0x02) and Media Element Item(0x03). The virtual filesystem containing the media content of the browsed player.
...
typedef void(*BtpBrowsingItemsReportCB)(uint16 aclHandle, uint08 pduID, uint08 status, uint08 itemType, uint08 itemNumb, void *pItemData);
...
/******************************************************************************
 * Function: btp_browsing_init
 * Descript: This interface be used by user to initial the browsing resource
...
*******************************************************************************/
int btp_browsing_sendGetFolderItemsCmd(uint16 aclHandle, uint08 scope, 
 ...
 *         If others value is returned means the send process fail.
*******************************************************************************/
int btp_browsing_sendGetTotalNumbOfItemsRsp(uint16 aclHandle, uint08 status,
