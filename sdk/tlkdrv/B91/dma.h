...
static inline void dma_config_abort(dma_chn_e chn ,dma_config_t *config)
{
...
//int g_dma_chn_timeout_en = 0;
static inline void dma_chn_dis_abort(dma_chn_e chn)
{
	if(reg_dma_ctr0(chn)&FLD_DMA_CHANNEL_ENABLE)
	{
		BM_SET(reg_dma_abt,BIT(chn));
		unsigned int t=stimer_get_tick();
		while(reg_dma_ctr0(chn)&FLD_DMA_CHANNEL_ENABLE)
		{
			if(clock_time_exceed(t,260))
			{
//				g_dma_chn_timeout_en=1;//indicate timeout
				break;
			}
		}
		dma_clr_abt_irq_status(BIT(chn));
	}
}
