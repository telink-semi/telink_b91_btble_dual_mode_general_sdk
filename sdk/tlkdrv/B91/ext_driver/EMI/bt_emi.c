...
#include "../BT/rf_bt.h"
#include "bt_emi.h"
#include "tlkdrv/B91/reg_include/soc.h"
...
unsigned short bt_pwr_list[BT_PWR_LIST_MAX_LENGTH] = {
		//scale << 8 | slice
		//BR
		1,//RF_TP_POWER_INDEX_N17p44dBm,
		2,//RF_TP_POWER_INDEX_N10p44dBm,
		4,//RF_TP_POWER_INDEX_N7p44dBm,
		6,//RF_TP_POWER_INDEX_N4p77dBm,
		9,//RF_TP_POWER_INDEX_N1p37dBm
		13,//RF_TP_POWER_INDEX_P2p72dBm,
		20,//RF_TP_POWER_INDEX_P5p0dBm,
		32,//RF_TP_POWER_INDEX_P7p0dBm,
		56,//RF_TP_POWER_INDEX_P8p0dBm,
//		37,
		63,//RF_TP_POWER_INDEX_P9P0_dBm,
		//EDR
		8 << 8 | 23,//RF_IQ_POWER_INDEX_N1P5_dBm,
		14 << 8 | 59,//RF_IQ_POWER_INDEX_p1P0_dBm,
		16 << 8 | 63,//RF_IQ_POWER_INDEX_p1P5_dBm
};
...
void rf_bt_rx_init(unsigned char _mode, signed short _chn)
{
	...
	rf_set_chn(_chn);
	//set bd address
	sub_wr_word(CS_ADDR+0x8,0x8674af00,31,0); //bdaddr[31:0]  00  af  74  86  76  05
	...
	sub_wr_word(CS_ADDR+0x4,_mode,27,27); //br:0, edr:1

	if(_mode == 1)	// add by pengcheng, suggest by xuqiang, 20201103
	{
		write_reg8(MODEMADDR+0xa0,0x1e); //br sync thre [31:0]
	}
	...
	// clear rx_done
	sub_wr_word(RD0_ADDR,0x0,15,15);
	// start ET0
...
//	write_reg32(BT_ADDR,0x40000000); //reset master_clock domain
	...
	sub_wr_word(TD0_ADDR+0x4,PktLength,12,3); //txlength[9:0]
...
//		gpio_set_level(GPIO_PD0,0);
	...
	if(BTMode == BT_MODE_BR)
	{
		btc_rf_tx_iq_tp_by_txtype_set(RADIO_TX_MODE_TP);
	}
	else
	{
		btc_rf_tx_iq_tp_by_txtype_set(RADIO_TX_MODE_IQ);
	}
	sub_wr(MODEMADDR+0x91,scale&0x1f,4,0);
...
//	write_reg32(BT_ADDR,0x40000000); //reset master_clock domain
	...
	sub_wr_word(ET_ADDR + ET0,0x0,5,3); //status[2:0]
	// clear tx_done
	sub_wr_word(TD0_ADDR,0x0,15,15);
	// start ET0
	write_reg32(BT_ADDR+0x100,0x80000000);
}
...
//		unsigned int rawstep = fst_rawstep + (nslot * (*finish_times));
		sub_wr_word(ET_ADDR + ET0,rawstep,31,16); //rawstp[15:0]
		sub_wr_word(ET_ADDR + ET0+0x4,(rawstep>>16)&0xfff,11,0); //rawstp[27:16]
		// clear status
		sub_wr_word(ET_ADDR + ET0,0x0,5,3); //status[2:0]
		// clear tx_done
...
//		unsigned int rawstep = bt_hw_get_et_next_slot(0)+1;
		sub_wr_word(ET_ADDR + ET0,rawstep,31,16); //rawstp[15:0]
		sub_wr_word(ET_ADDR + ET0+0x4,(rawstep>>16)&0xfff,11,0); //rawstp[27:16]
		sub_wr_word(ET_ADDR + ET0+0x4,finestp,25,16);//finestp[9:0]
