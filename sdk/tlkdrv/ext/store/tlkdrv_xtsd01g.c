...
static int tlkdrv_xtsd01g_open(void)
{
	...
	//enable spi mode
	while(--retry){
		r1 = tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_00, 0, 0x95);
		if(r1 == 0x01) break;
		delay_us(100);
	}
	if(r1 != 0x01){
	...
	retry=500;
	do{
		tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_55,0,1);
		delay_us(10);
		r1 = tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_41, 0x40000000, 0X01);
		delay_ms(1);
	}while(r1 && retry--);
	...
	r1 = tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_58,0,0X01);
	if(r1==0){
...
int tlkdrv_xtsd01g_readBlock(uint08 *pBuff, uint32 blkOffs)
{
	...
	r1 = tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_17, blkOffs, 0X01);
	if(r1==0){
...
int tlkdrv_xtsd01g_writeBlock(uint08 *pData, uint32 blkOffs)
{
	...
	r1 = tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_24, blkOffs, 0X01);
	if(r1 == 0){
...
int tlkdrv_xtsd01g_readBlocks(uint08 *pBuff, uint32 blkOffs, uint16 blkNumb)
{
	int ret;
...
int tlkdrv_xtsd01g_writeBlocks(uint08 *pData, uint32 blkOffs, uint16 blkNumb)
{
	int ret;
...
bool tlkdrv_xtsd01g_getCID(uint08 *cid_data)
{
    ...
    r1 = tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_10,0,0x01);
    if(r1==0x00){
...
bool tlkdrv_xtsd01g_getCSD(uint08 *csd_data)
{
    uint08 r1;
	uint08 buffer[32];
    r1=tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_09,0,0x01);
    if(r1==0){
		r1 = tlkdrv_xtsd01g_spiRecvData(buffer,16);
...
void tlkdrv_xtsd01g_eraseBlocks(uint32 blkOffs, uint32 blkNumb)
{
	...
	resp=tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_32, start_addr, 0x01);
	if(resp != 0){
		tlkdrv_xtsd01g_spiCsHigh();
		return;
	}
	
	resp=tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_33, end_addr, 0x01);
	if(resp != 0){
		tlkdrv_xtsd01g_spiCsHigh();
		return;
	}
		
	tlkdrv_xtsd01g_sendCmd(TLKDEV_XTSD01G_CMD_38, 0x0, 0x01);	//resp = 0xC0
...
static uint08 tlkdrv_xtsd01g_spiGetResponse(uint08 Response)
{
	...
	uint32 Count = 0x3F;
	do{
		if(tlkdrv_xtsd01g_readByte() == Response) return TLKDEV_XTSD01G_MSD_RESPONSE_NO_ERR;
		delay_us(100);
		t ++;
	}while(t < Count);
	#if(TLKDEV_XTSD01G_DEBUG_ENABLE)
...
static uint08 tlkdrv_xtsd01g_sendCmd(uint08 cmd, uint32 arg, uint08 crc)
{
	...
	if(cmd==TLKDEV_XTSD01G_CMD_12) tlkdrv_xtsd01g_writeByte(0xff);
...
static void tlkdrv_xtsd01g_readData(uint08 *pBuff, uint32 buffLen)
{
	...
	if(((buffLen % 4) == 0) && (buffLen>=4))
	{
		...
		tlkdrv_xtsd01g_spiReadDma(TLKDEV_XTSD01G_SPI_MODULE, pBuff, buffLen);
	}
	else
	#endif
	{
		TLKDEV_XTSD01G_IRQ_DISABLE();
		tlkdrv_xtsd01g_spiRead(TLKDEV_XTSD01G_SPI_MODULE, pBuff, buffLen);
...
static const uint08 scTlkDevXtsd01gDiskDBR[64] =
{
	...
	TLKDRV_XTSD01G_DISK_RSV_NUMB & 0xFF, (TLKDRV_XTSD01G_DISK_RSV_NUMB & 0xFF00) >> 8, //Number of reserved sectors, 8 (14~15)
	...
	(TLKDRV_XTSD01G_DISK_FAT_NUMB & 0xFF), (TLKDRV_XTSD01G_DISK_FAT_NUMB & 0xFF00)>>8, //Number of sectors per FAT, 64 (22~23)
...
int tlkdrv_xtsd01g_diskFormat(void)
{
	...
	tmemset(block, 0, TLKDRV_XTSD01G_DISK_BLOCK_SIZE);
	for(index = 1; index < TLKDRV_XTSD01G_DISK_RSV_NUMB; index ++){
	...
	for(fatNum = 0; fatNum < TLKDRV_XTSD01G_DISK_FAT_COPIES; fatNum++){
		...
		tlkdrv_xtsd01g_diskWrite(block, offset);
		tmemset(block, 0, TLKDRV_XTSD01G_DISK_BLOCK_SIZE);
		for(index = 1; index < TLKDRV_XTSD01G_DISK_FAT_NUMB; index ++){
			tlkdrv_xtsd01g_diskWrite(block, offset+index);
		}
		offset += index;
	}
