...
int tlkmmi_rdt_leCoreInit(void)
{
	...
	tlkapi_flash_write(TLK_CFG_FLASH_LE_ADDR_ADDR, leAddr, 12);
...
static int tlkmmi_rdt_leCoreEventCB(uint32 evtID, uint08 *pData, int dataLen)
{
	...
	else if(evtCode == HCI_EVT_LE_META)  //LE Event
	{
		...
		else if (subEvt_code == HCI_SUB_EVT_LE_CONNECTION_UPDATE_COMPLETE)	// connection update
		{
			...
			tlkapi_trace(TLKMMI_RDT_DBG_FLAG, TLKMMI_RDT_DBG_SIGN, "ble connect update: connInterval-0X%02X connLatency-0X%02X Timeout-0X%02X", pUpt->connInterval,pUpt->connLatency,pUpt->supervisionTimeout);
