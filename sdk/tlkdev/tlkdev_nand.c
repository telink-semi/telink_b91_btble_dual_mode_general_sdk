...
int tlkdev_nand_init(void)
{
	sTlkDevNandIsReady = false;
		...
		if((sdcardfatblocknum != (sdcardblocknum - SDDISK_SECTOR_OFFSET))){
			...
			tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "sdcard format end, please restart");
		}
	#endif
...
void tlkdev_nand_test(void)
{
	...
	tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "nand_flash_pressure_test_start");

	tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "tlkdev_xtsd04g_read");
	...
	sector_addr = 1002;
	tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "tlkdev_xtsd04g_write");
	if(tlkdev_xtsd04g_write((unsigned char * )(test_addr),sector_addr,250))
	{
		tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "SD_WriteDisk_fail");
		return;
	}
	else
	{
		tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "SD_WriteDisk_ok");
	}
	...
	for(unsigned int i = 0; i < 250; i+=10)
	{
		...
		for(unsigned int j = 0; j < (512*10); j++)
		{
			if(temp[j] != (*(unsigned char * )(test_addr+j+(i*512))))
			{
				tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "SD_ReadDisk_error_inaddr:addr-0x%x,i%d,j:%d", test_addr, i, j);
				return;
			}
		}
		tlkapi_trace(TLKDEV_NAND_DBG_FLAG, TLKDEV_NAND_DBG_FLAG, "SD_ReadDisk_ok_inaddr:");
	}
	#endif
}
