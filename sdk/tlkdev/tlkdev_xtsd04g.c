...
#define TLKDEV_XTSD04G_DEBUG_ENABLE      0
...
int tlkdev_xtsd04g_init(void)
{
	...
	if(sTlkDevXtsd04gEnterFlag != 0){
		...
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "sd_nand_flash_init Reentry", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some logic error is happen, the function is Reentry when sd nand flash is using in other function ", 0, 0);
	...
	if(r1 != 0x01){
		#if(TLKDEV_XTSD04G_DEBUG_ENABLE)
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "sd_nand_flash_init", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "enable spi mode fail", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some error is happen,Please keep the site and contact telink", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
		#endif
	...
	if(retry == 0){
		#if(TLKDEV_XTSD04G_DEBUG_ENABLE)
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "sd_nand_flash_init", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "wait for busy fail", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some error is happen,Please keep the site and contact telink", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
		#endif
		tlkdev_xtsd04g_spiDisSelect();
...
int tlkdev_xtsd04g_read(uint08 *pBuff, uint32 sector, uint08 sectCnt)
{
	if(sTlkDevXtsd04gEnterFlag != 0){
		#if(TLKDEV_XTSD04G_DEBUG_ENABLE)
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "tlkdev_xtsd04g_read Reentry", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some logic error is happen, the function is Reentry when sd nand flash is using in other function ", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
		#endif
		return 0x11;
...
int tlkdev_xtsd04g_write(uint08 *pData, uint32 sector, uint08 sectCnt)
{
	if(sTlkDevXtsd04gEnterFlag != 0){
		#if(TLKDEV_XTSD04G_DEBUG_ENABLE)
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "tlkdev_xtsd04g_write Reentry", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some logic error is happen, the function is Reentry when sd nand flash is using in other function ", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
		#endif
		return 0x11;
...
uint32 tlkdev_xtsd04g_getSectorCount(void)
{
	...
	{
		#if(TLKDEV_XTSD04G_DEBUG_ENABLE)
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "tlkdev_xtsd04g_getSectorCount Reentry", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some logic error is happen, the function is Reentry when sd nand flash is using in other function ", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
		#endif
		return 0x11;
...
static uint08 tlkdev_xtsd04g_spiGetResponse(uint08 Response)
{
	...
	#if(TLKDEV_XTSD04G_DEBUG_ENABLE)
	if(t == Count){
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "sd_nand_flash", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "tlkdev_xtsd04g_spiGetResponse", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some error is happen,Please keep the site and contact telink", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
	}
	#endif
...
static uint08 tlkdev_xtsd04g_sendCmd(uint08 cmd, uint32 arg, uint08 crc)
{
	...
	if(t == Count){
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "sd_nand_flash", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "tlkdev_xtsd04g_sendCmd", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some error is happen,Please keep the site and contact telink", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
	}
	#endif
...
uint08 tlkdev_xtsd04g_waitReady(void)
{
	...
	if(t == Count){
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "sd_nand_flash", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "tlkdev_xtsd04g_waitReady", 0, 0);
		my_dump_str_data(TLKDEV_CFG_DBG_ENABLE, "maybe some error is happen,Please keep the site and contact telink", 0, 0);
		while(1) tlkapi_debug_process(); /* open this for usb log printf*/
	}
	#endif
